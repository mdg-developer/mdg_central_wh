...
class StockQuant(models.Model):
    ...
    def _apply_inventory(self):
        ...
        loc =['MDGCWH2-Sellable','MDGCWHB-Sellable']
        cwh_location = models.execute_kw(db, sd_uid, password, 'stock.location', 'search',
                                         [[['name', 'in', loc], ['is_cwh_location', '=', True]]],
                                         {'limit': 1})
        inv_loss_location = models.execute_kw(db, sd_uid, password, 'stock.location', 'search',
                                              [[['name', '=', 'Inventory loss']]], {'limit': 1})
        for record in self:
            location = record.location_id
            product = record.product_id
            product_uom = record.product_uom_id.name
            counted_quantity = record.inventory_quantity
            diff_quantity = record.inventory_diff_quantity
